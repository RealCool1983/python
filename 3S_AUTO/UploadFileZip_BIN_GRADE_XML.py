...
def writeBat(sXmlPath):
    ...
    sReadPath1 = os.path.join(sReadPath1, "BUILD_BIN_GRADE.bat") 
...
def runVC6AutoBuild(sXmlPath):
    ...
    sReleaseFolder = os.path.join(sPC_NewMPUI_Path,"src\\Release\\BIN_GRADE.exe")
...
def runUacBat(sXmlPath):
    ...
    sUacFolder = os.path.join(sPC_NewMPUI_Path,"src\\UAC\\BIN_GRADE.exe")
...
def runCopyFromGit(sXmlPath):
    ...
    ignore_dirs = shutil.ignore_patterns( '.gitignore', '.git', 'releaseNote')
...
def runVCVersion(sXmlPath):
    ...
    sPath = os.path.join(sPC_NewMPUI_Path, 'src\BIN_GRADE.rc')
    print(sPath)
...
def parseXML():
    ...
    xmlPath = r"D:\\3S_PC\\python\\3S_AUTO\\BIN_GRADE.xml"
